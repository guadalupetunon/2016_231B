...
a. Using matrix manipulation, calculate the regression residuals. Show the average of the residuals is zero and find $\hat{\sigma}$ based on sum of squared residuals, divided by $n-p$.
...
Do you see any pattern? Should you see a pattern? Why or why not? For the latter, do you see a pattern?  
Would a pattern be evidence against the modeling assumption (e.g., constant variance of $\sigma^2$)?
...
- When $\mu$ is zero, all the expression is $0$ and our regression of $Y$ on $X$ captures the real DGP.
...
- sigmaX = the true variance-covariance matrix of $X$

- betas = true parameters of the model

- sigmae = true standard deviation of the epsilon
...
ovb_sim <- function(n=100, muX=c(0,0,0), sigmaX, betas, sigmae, omitted=FALSE){
...
# For all models, let's also define the true stadard deviation of the error term
sigma <- sqrt(2)
```
---
...
1. The three independent variables belong in the real model and they are correlated -- and we specify the right regression.
```{r}
reg1 <- replicate(10000, ovb_sim(n=100, muX=c(0,0,0), sigmaX=SigmaX_with_positive, 
                                 betas=c(1,2,4,3), sigmae=sigma, omitted=FALSE))
```

2. The three independent variables belong in the real model and they are correlated (positive covariance) -- our regression omits $x_3$

```{r}
reg2 <- replicate(10000, ovb_sim(n=100, muX=c(0,0,0), sigmaX=SigmaX_with_positive, 
                                 betas=c(1,2,4,3), sigmae=sigma, omitted=TRUE))
```
...
```{r}
reg3 <- replicate(10000, ovb_sim(n=100, muX=c(0,0,0), sigmaX=SigmaX_with_negative, 
                                 betas=c(1,2,4,3), sigmae=sigma, omitted=TRUE))
```
...
```{r}
reg4 <- replicate(10000, ovb_sim(n=100, muX=c(0,0,0), sigmaX=SigmaX_without, 
                                 betas=c(1,2,4,3), sigmae=sigma, omitted=TRUE))
```
...
Why is the sampling distributions of the estimators tighter for the first simulation (slide 23) than in in this plot? 
Hint: what is the disturbance variability in each case?
