...
# read.csv [comma], read.table [tab,space], read.dta [stata], read.xpt [sas], read.spss [.sav for spss]
p <-scan()
1
2
3 
...
data <- read.csv("exampleData1.csv")
class(data)
names(data)
rownames(data)
...
X=as.matrix(data[,1:(dim(data)[2]/2)])
Y=as.matrix(data[,(dim(data)[2]/2+1):dim(data)[2]])
dim(X)
dim(Y)
