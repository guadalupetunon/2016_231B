...
- So far, we have covered methods for hypothesis testing that require information about the population distribution and/or analytical formulas (there is one exception for this, what is it?)

- But sometimes we don't know how the population looks like or there are no analytical formulas to estimate the uncertainty of point estimates.

- For that, we use the bootstrap---we approximate the population distribution by using the information in the sample. 
...
- The sampling distribution is derived by using the sample.

- We draw samples with replacement from the data and, for each simulated sample, store the statistic of interest.

- The histogram of these simulation statistics estimates the sampling distribution of the estimator. 
...
Say we collected the following simple random sample:
...
```{r}
y_bart <- trimmed.mean(y, k=3)
y_bart
```

How can we calculate a CI for the trimmed mean?
...
How do we calculate the SE of our observed trimmed mean?
...
We will use the `swiss` dataset to run a regression of `education` on `infant mortality`. We will then use the bootstrap to calculate $SE(\hat{\beta})$. Note that to use the bootstrap we need to assume a model (for the sake of the exercise, we will believe this assumption despite it is not very credible in this context).
...
We will now use the bootstrap to calculate $SE(\hat{\beta})$.
```{r}
betaboot <- rep(NA,B)
...
Fixed-effects models help to isolate the effects of \textit{within-unit} variation---though coefficients are pooled across units.

$$ Y_{it}= \alpha_i + \beta D_{it} + \epsilon_{it} $$ where there is one dummy variable (``fixed effect") $\alpha_i$ for each unit $i$ (here unit=country).
...
scatterplot(yhat~Panel$x1|Panel$country,
            xlab="x1", ylab="yhat")
abline(lm(Panel$y~Panel$x1), lwd=5, col="black")
